        ...
        for po in protoObj:
            ...
            for fielddesc, value in po.ListFields():
                if fielddesc.type == descriptor.FieldDescriptor.TYPE_GROUP \
                        or isinstance(value, RepeatedCompositeFieldContainer) \
...
class GooglePlayAPI(ProtobufMixin):
    """
    ...
    def download(self, packageName, versionCode, offerType=1):
        ...
        message = self.toDict(message)

        if 'commands' in message and 'displayErrorMessage' in message['commands']:
            raise DownloadingError(message['commands']['displayErrorMessage'])

        buyResponse = message.get('payload', {}).get('buyResponse', {})
        appDeliveryData = buyResponse.get('purchaseStatusResponse', {}).get('appDeliveryData', None)

        if appDeliveryData is None:
            raise DownloadingError('appDeliveryData is None')

        # url = message.payload.buyResponse.purchaseStatusResponse.appDeliveryData.downloadUrl
        url = appDeliveryData.get('downloadUrl', None)

        if self.debug:
            print 'Download url: {}'.format(url)

        # cookie = message.payload.buyResponse.purchaseStatusResponse.appDeliveryData.downloadAuthCookie[0]
        cookie = appDeliveryData.get('downloadAuthCookie', [None, ])[0]
